// Screenshot harness for Flow B (KYC), NOT covered by test/shots.dart (which
// only captures kyc_intro and kyc_bvn — 2 of the ~9 real screens in this
// flow). The suitability questionnaire/result screens this file used to also
// cover are gone (R-51, DECISIONS.md, 2026-08-31). Router gating only checks
// state.signedIn (see app_router.dart's _gateRedirect) — once true, KYC
// routes free-roam regardless of kycSubmitted/kycApproved, so those two flags
// are set per-screen to match what a real user actually has AT that screen
// (kycSubmitted flips true on next_of_kin.dart itself; kycApproved on
// approved.dart itself).
...
// (route, kycSubmitted, kycApproved) — signedIn is always true for this
// flow (set once in _mount).
const _routes = <String, (String, bool, bool)>{
  '13_kyc_intro': (Routes.kycIntro, false, false),
  '14_bvn_nin': (Routes.kycBvn, false, false),
  '15_chn': (Routes.kycChn, false, false),
  '16_id_upload': (Routes.kycId, false, false),
  '17_liveness': (Routes.kycLiveness, false, false),
  '18_utility_bill': (Routes.kycUtilityBill, false, false),
  '19_bank_dcs': (Routes.kycBankDcs, false, false),
  '20_declarations': (Routes.kycDeclarations, false, false),
  '21_next_of_kin': (Routes.kycNextOfKin, false, false),
  ...
  '23_24_submitted_pending': (Routes.kycSubmitted, true, false),
  '25_approved': (Routes.kycApproved, true, false),
  '26_outcome_not_approved': (Routes.kycOutcome, true, false),
  // '27_questionnaire'/'28_suitability_result'/'29_terms_of_service' no
  // longer exist (R-51, DECISIONS.md, 2026-08-31) — the suitability
  // questionnaire, its result screen and the legal-documents chain they used
  // to hand off to are all removed.
};
...
void main() {
  ...
  for (final entry in _routes.entries) {
    testWidgets('capture ${entry.key}', (tester) async {
      ...
      final (route, kycSubmitted, kycApproved) = entry.value;

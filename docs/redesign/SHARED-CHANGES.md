...
| S-10 | wave · kyc checklist (`s11`) | `lib/data/repositories/kyc_repository.dart` + backend | `declarations_screen.dart`'s (`s19`) second question — "Do you work for a stockbroker or the NGX?" — has no backend field at all (only `pepSelfDeclared` is persisted; confirmed against `UpdateKycDraftFieldsRequest`/`KycSubmission`). Filed properly in `BACKEND_GAPS.md`; noted here too because `kyc_checklist_screen.dart`'s own "Declarations done" signal can only check the PEP half for this reason — see that file's header comment. | ✅ resolved — question removed entirely, owner ruling 2026-09-01 (it "is not a PEP question and it goes"); see `BACKEND_GAPS.md`'s now-resolved s19 entry |
...
| D-7 | `lib/screens/onboarding/personal_details_screen.dart` | RESOLVED 2026-08-29 (A-1 product-owner audit — "why is there a few more details screen? why is that still there"). Was: R-19 audit found its Residential address/City/State fields duplicated `utility_bill.dart`'s (`s17`) same PATCH target, and (at the time) assumed DOB/phone were still uniquely captured here. Since then: sign_up_screen.dart gained its own phone field (BR-3), and BR-4 gave the BVN/NIN registry a resolved DOB — closing the "no other writer" argument for both remaining fields. `kyc_intro.dart` no longer detours through this screen at all: DOB (the one field genuinely still needing a home when the registry doesn't resolve one and the account has none on file) now folds into `bvn_nin.dart`'s `s13` confirm step instead. This screen is kept per the audit's own "do not delete" instruction, reported as a removal candidate — its route stays registered/walked by `test/route_walk_test.dart` since it's still directly reachable (a deep link, `avatar_screen.dart`'s own back arrow), just no longer part of any screen's forward navigation. |
...
|---|---|---|---|
| N-1 | `s11` | KYC checklist hub | Functionally distinct from `s10`: `s10` is a one-shot entry with resume logic, `s11` is the resume point for an investor re-entering KYC part-way. Merging them into `kyc_intro.dart` would break the resume path. Needs its own screen and route. Found by the `s10`/`s12` agent. **2026-08-29 (A-4 audit fix):** this row originally said `s11` "is re-entered after **every** completed step" — that was the X-5 directive this same file's history shows, and it's exactly what the 2026-08-29 product-owner audit flagged as two of the KYC flow's "foolish screens that can interrupt when you want to go to the next thing." `chn_screen.dart`/`checking.dart`/`bank_dcs_screen.dart`/`declarations_screen.dart` now advance straight to the real next step (via `kyc_checklist_screen.dart`'s new `nextKycStepRoute`, the same derivation the hub's own UI already used) instead of routing here — the hub itself is unchanged and still exists solely as the resume point described above. |
...
| S-6 | `KycSubmissionStatus` resolved name/dob/phone (BR-4 landed) | Added optional `resolvedName`/`resolvedDob`/`resolvedPhone` (`String?`) to `KycSubmissionStatus` and `_fromJson` (`lib/data/repositories/kyc_repository.dart`), parsed straight off `draftStep1`'s response per BR-4's wire shape. No separate name-match field was needed — the backend's own doc comment on `resolvedName` confirms the existing `verificationSignals.name` boolean already IS that computed result. Wired into `bvn_nin.dart`'s confirm step (`s13`) the same pass — see its own header and `BACKEND_GAPS.md`'s now-resolved s13 entry. |

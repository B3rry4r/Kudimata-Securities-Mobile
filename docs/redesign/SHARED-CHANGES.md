...
|---|---|---|---|
| F-1 | **R-1a** — suitability moves to after OTP, before the legal documents | `otp_screen.dart` → questionnaire; `app_router.dart` gated-flow order; `app_state.dart:485`'s post-KYC trading gate removed as the entry point | **NOT DONE.** Ruled 2026-08-26, survived four waves unexecuted. |
| F-2 | new screens need routes | `s11` KYC checklist hub and `s07` post-signup overview are being built this wave with no route — both agents file requests rather than wiring to a constant that does not exist | pending those agents |
